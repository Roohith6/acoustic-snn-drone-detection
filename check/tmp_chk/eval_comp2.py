...
amb = np.load('synthetic_sample.npy')
drn = np.load('sample_drone_thresholds.npy')

def eval_ann(thresh):
    # thresh is in 0..65535, representing X * 0.9
    X = thresh / 65535.0 / 0.9
    z1 = X @ W1 + b1
    a1 = np.maximum(0, z1)
    z2 = a1 @ W2 + b2
    a2 = np.maximum(0, z2)
...
print("Synthetic ANN:", eval_ann(amb))
print("Drone ANN:", eval_ann(drn))
...
def eval_snn(thresh, clamp=True):
    ...
    for t in range(50):
        ...
        if clamp: v_hid = np.maximum(0, v_hid)
        ...
        if clamp: v_out = np.maximum(0, v_out)
    return out_spikes

print("\nCLAMP=True")
print("Synthetic SNN:", eval_snn(amb, True))
print("Drone SNN:", eval_snn(drn, True))

print("\nCLAMP=False")
print("Synthetic SNN:", eval_snn(amb, False))
print("Drone SNN:", eval_snn(drn, False))
