...
# Use a stratified subset for tuning
np.random.seed(42)
...
thresholds_int = (np.clip(X_tune * 0.9, 0.0, 1.0) * 65535).astype(np.uint32)

def test_scale(scale):
    V_thresh_h = int(np.round(a1_max * W1_scale * scale))
    y_pred = []
    for i in range(len(X_tune)):
    ...
    acc = accuracy_score(y_tune, y_pred)
    f1 = f1_score(y_tune, y_pred, zero_division=0)
    print(f"Scale {scale:.2f}: Acc={acc:.4f} F1={f1:.4f}")
