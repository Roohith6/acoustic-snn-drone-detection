...
| Session 12 | User added `neuro.sdc`, recompiled | **Timing now PASSES at 50MHz with positive slack in every corner** (Slow 85°C: +2.046ns, Slow 0°C: +3.712ns, Fast 0°C: +11.265ns) — real, meaningful confirmation this time, since `neuro.sdc` was correctly read (`Info (332104): Reading SDC File`) |
| Session 12 | First `combine_scale.v` fix (Session 11) **did not actually resolve** the constant-overflow warning — same warning, same line, in the next compile | My earlier stated mechanism was wrong: Quartus checks a sized-signed-decimal literal's stated magnitude against the representable signed range *before* any wraparound logic, so `16'sd32768` (magnitude 32768 > max +32767 for 16-bit signed) fails the same way `-16'sd32768` did, just for a different specific reason. No sized-signed-decimal literal can express -32768 directly. |
| Session 12 | Corrected fix: plain default-width literal (`-32768`, no explicit size/sign literal syntax), truncated to the 16-bit target on assignment — sidesteps the sized-literal range check entirely | Pushed, not yet recompiled/confirmed — next step |
...
### 6.1 Code fix from synthesis review — took two attempts, documented honestly
`combine_scale.v` line 61 had `-16'sd32768` — flagged by `Warning (10259):
constant value overflow`.

**First attempt (Session 11) did not work.** Reasoned that `16'sd32768`'s
bit pattern already equals -32768 in two's complement, so removing the
leading minus should fix it — wrong: the *next* compile (Session 12)
showed the identical warning on the identical line. The actual mechanism:
Quartus checks a sized-signed-decimal literal's **stated decimal
magnitude** against the representable signed range (±32767 for 16 bits)
*before* any wraparound/two's-complement logic applies — so `16'sd32768`
(magnitude 32768) fails the same check `-16'sd32768` did, for a related
but distinct reason. No sized-signed-decimal literal can express -32768
directly in Verilog/Quartus's interpretation of that syntax.

**Working fix (Session 12):** a plain, unsized default-width literal
(`-32768`, no explicit `16'sd...`), truncated to the 16-bit target on
assignment. This sidesteps the sized-literal range check entirely — the
literal itself is comfortably representable in Verilog's default
(32-bit signed) width, and standard truncation-on-assignment correctly
produces the intended `16'h8000` bit pattern. Not yet recompiled/confirmed
— next step.

### 6.2 Timing — CONFIRMED PASSING at 50MHz (Session 12)
With `neuro.sdc` correctly picked up (`Reading SDC File: 'neuro.sdc'`
confirmed in the log), TimeQuest now reports **positive setup and hold
slack in every process/voltage/temperature corner analyzed:**
```
Slow 1200mV 85C:  worst-case setup slack +2.046ns   worst-case hold slack +0.386ns
Slow 1200mV  0C:  worst-case setup slack +3.712ns   worst-case hold slack +0.352ns
Fast 1200mV  0C:  worst-case setup slack +11.265ns  worst-case hold slack +0.170ns
```
This is a real, meaningful result — the design **meets 50MHz (20ns
period)** with margin to spare even in the slow/hot worst-case corner
(the one that matters most for actual silicon). The earlier -15.675ns
"failure" reported before `neuro.sdc` existed was entirely an artifact of
the unconstrained 1GHz default check, not a real problem — confirmed now
by this constrained run.

Note: this was still run under Fitter `Auto Fit` (reduced optimization
effort — see `Info (170199)/(170200)`: "Optimizations that may affect the
design's timing were skipped"). The positive margin already achieved
under reduced effort is a good sign; a full-effort compile (once
available/licensed) would likely preserve or improve this margin, not
threaten it.
...
11. ~~First Quartus synthesis attempt~~ — DONE (§6): 0 errors, fit succeeded.
12. ~~Add `neuro.sdc`, confirm real timing~~ — DONE: **50MHz PASSES with positive slack in every corner** (§6.2). Real, meaningful result.
13. **User:** apply the corrected `combine_scale.v` (second attempt — first one didn't actually work, see §6.1) and recompile once more, to get a fully clean warning log.
...
15. Since timing already comfortably passes at 50MHz: decide whether `spike_gated_mac.v`'s sequential-MAC speed (§4.11) is even worth addressing now (it's a resource/inference-time concern, not currently a timing-closure blocker), or whether the design can proceed toward pin planning / board bring-up as-is.
