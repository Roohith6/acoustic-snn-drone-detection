...
module rate_encoder #(
    ...
    input  wire [1:0] sample_sel, // which of 4 pre-loaded threshold sets to use
    ...
    // ---- threshold ROM ----
    // Four threshold sets, one per selectable sample. All four are read
    // in parallel every cycle (cheap -- just 4 small ROMs); sample_sel picks
    // which one actually feeds the comparator. Held constant during a run
    // by the board wrapper, so no need to worry about it changing mid-pass.
    wire [WIDTH-1:0] threshold_data_0, threshold_data_1, threshold_data_2, threshold_data_3;
    wire [WIDTH-1:0] threshold_data;

    threshold_mem #(.N_NEURONS(N_NEURONS), .WIDTH(WIDTH), .MEM_FILE(THRESH_MEM_FILE_0))
        u_thresh0 (.clk(clk), .addr(cnt_n), .data_out(threshold_data_0));
    threshold_mem #(.N_NEURONS(N_NEURONS), .WIDTH(WIDTH), .MEM_FILE(THRESH_MEM_FILE_1))
        u_thresh1 (.clk(clk), .addr(cnt_n), .data_out(threshold_data_1));
    threshold_mem #(.N_NEURONS(N_NEURONS), .WIDTH(WIDTH), .MEM_FILE(THRESH_MEM_FILE_2))
        u_thresh2 (.clk(clk), .addr(cnt_n), .data_out(threshold_data_2));
    threshold_mem #(.N_NEURONS(N_NEURONS), .WIDTH(WIDTH), .MEM_FILE(THRESH_MEM_FILE_3))
        u_thresh3 (.clk(clk), .addr(cnt_n), .data_out(threshold_data_3));

    assign threshold_data = (sample_sel == 2'd0) ? threshold_data_0 :
                             (sample_sel == 2'd1) ? threshold_data_1 :
                             (sample_sel == 2'd2) ? threshold_data_2 :
                                                     threshold_data_3;
