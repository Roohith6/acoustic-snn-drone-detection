...
## 7. Immediate next actions (proposed order) — updated Session 10
...
10. ~~Run full 48-sample batch through actual RTL simulation~~ — DONE: **48/48 bit-exact, 47/48 accuracy, fully RTL-proven** (§4.15).
11. **Core RTL is complete and now proven across a real batch, not just 2 hand-picked samples.** Remaining work is optimization and hardware bring-up:
    - Address the two documented "simplest correct first" shortcuts before serious performance/resource numbers matter: `spike_gated_mac.v`'s fully-sequential one-read-per-cycle design (§4.11) and `snn_top.v`'s hardcoded pipeline-latency wait (§4.14).
    - **Need from you (optional, strengthens confidence further):** a larger batch (100+) for a tighter statistical read on real accuracy — 48 gave a clear, useful signal, more would narrow the confidence interval, especially valuable for a hackathon writeup.
    - Phase 6–7: synthesis (Quartus, per the original DE2-115 target), resource/timing measurement, FPGA bring-up. This is the next genuinely new phase of work — everything above is algorithm/RTL-correctness work, which is now done.
