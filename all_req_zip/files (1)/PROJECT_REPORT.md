...
## 7. Immediate next actions (proposed order) — updated Session 5
1. ~~Pick path A/B, get biases~~ — DONE.
2. ~~Implement `rate_encoder.v`~~ — DONE (§4.8–4.9).
3. ~~Implement weight memory modules~~ — DONE (§4.3–4.4, §4.6).
4. ~~Design + implement `sigmoid_lut.v`~~ — DONE, verified against 256 real values (§4.10).
5. ~~Implement `spike_gated_mac.v`~~ — DONE, verified against both real samples' full 50-timestep sequences (§4.11).
6. Combine: `raw_acc` (from spike_gated_mac, in W1's fixed-point units) needs rescaling — divide by `T=50`, convert to a common fixed-point base with `b1` (which has its *own*, different scale: 3814.7672 vs W1's 5515.9670), add, then feed into `sigmoid_lut.v`. This rescale/combine step is the next real design decision (a `hidden_neuron.v` wrapper) — not yet designed.
7. Verify `hidden_neuron.v`'s full `z1→sigmoid→a1` output against real Python-computed `a1` for both samples (128 values × 2 — reuses the same real data already on hand).
8. Repeat the accumulate pattern for the output layer: `W2`/`b2`-gated accumulate on the (now real, RTL-computed) hidden spikes/activations → argmax (softmax skipped, monotonic).
9. `snn_controller.v` (FSM) once all datapath modules are frozen.
10. `snn_top.v` — integration.
11. Phase 5: full golden-dataset regression (Tests 1–7, Section 35 of the original doc).
12. Phase 6–7: synthesis, resource/timing, FPGA bring-up.
