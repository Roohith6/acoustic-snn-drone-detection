...
//
// UPDATED Session 9: A_FIXED/B_FIXED below are derived from the FOLDED
// W1/b1 (input standardization -- see PROJECT_REPORT.md sec 5.0 -- folded
// in algebraically so no other module needed to change). These constants
// are tied to W1_folded's scale (199.6272) and b1_folded's scale
// (108.0679), NOT the original pre-fold scales. If the weights are
// retrained/re-exported, these constants must be recomputed to match the
// new .mem file headers' printed scale values -- they are not automatic.
// =============================================================================
module combine_scale #(
    parameter ACC_WIDTH = 32,
    parameter BIAS_WIDTH = 16,
    parameter signed [31:0] A_FIXED = 32'd13769562,    // 2^7 / (50 * 199.6272), Q?.30 -- FOLDED weights
    parameter signed [31:0] B_FIXED = 32'd1271783328,  // 2^7 / 108.0679,        Q?.30 -- FOLDED bias
    parameter SHIFT = 30
) (
    input  wire clk,
    ...
    always @(posedge clk) begin
        if (sum > SAT_MAX)
            ...
            z1_q <= 16'sd32768;  // bit pattern for 32768 IS -32768 in signed
                                  // 16-bit two's complement (no positive 32768
                                  // representation exists in 16 bits) -- the
                                  // previous "-16'sd32768" tried to negate an
                                  // already-most-negative value, which Quartus
                                  // correctly flagged as a constant overflow
                                  // (10259). Same numeric result either way,
                                  // fixed for correctness/cleanliness.
        else
            z1_q <= sum[15:0];
    end
